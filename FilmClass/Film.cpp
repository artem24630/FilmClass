...
{
    if (isNameCorrect(name2) && isFioCorrect(fioDirector2) && isFioCorrect(fioScreenwriter2) &&
        isGenreCorrect(genre2) &&
        isYearCorrect(year2)) {
        name_ = name2;
        genre_ = genre2;
        year_ = year2;
...
{
    if (myGenre.getLength() > 20 || !isBigLetter(myGenre.getCharAt(0)))
